...
{
    public class PutMoneyToTheAccount
    {
        public void Handle()
        {
            while (true)
            {
                Console.WriteLine("Enter the ID of replenishment account:");
                string? toAccountID = Console.ReadLine();

                Console.WriteLine("Enter the count of money to replenish the account:");
                string? money = Console.ReadLine();

                try
                {
                    ITransaction transaction = CentralBank.GetInstance().PutMoneyToTheAccount(
                        Guid.Parse(toAccountID ?? string.Empty),
